...
# Default dataset cache directory. Override via the HF_DATASETS_CACHE
# environment variable (falls back to the Hugging Face default if unset).
DEFAULT_CACHE_DIR = os.environ.get("HF_DATASETS_CACHE", None)
...
def translate_text(text: str, org_tok, alien_tok, direction: str) -> str:
    """Convert text between the original and alien tokenizers."""
...
def load_magpie_data(
    ...
    Returns:
        alien_corpus: list of alien-tokenized texts
...
def load_reference_corpus(
    ...
    Args:
        ...
        max_size: maximum number of texts (None uses the full corpus)
    ...
    if dataset_name.lower() == "tulu3":
        ...
        # Efficient extraction: iterate only as far as needed
        corpus = []
...
class BijectionExtractor:
    ...
    def extract_bijection(self, exclude_special_tokens: bool = True) -> Tuple[Dict[int, int], Dict[int, int]]:
        """Extract the bijection by matching identical token strings between the two tokenizers."""
...
class NGramAttack:
    ...
    def __init__(self, n: int = 2, top_k_tokens: int = None):
        """
        Args:
            n: n-gram size
            top_k_tokens: use only the top-k tokens by frequency (None uses all)
        """
        self.n = n
    ...
    def prepare_corpus(self,
        ...
        """Convert texts into token-ID sequences (using datasets.map)."""
    ...
    def compute_similarity_matrix(self) -> Tuple[np.ndarray, List[int], List[int]]:
        ...
        if self.top_k_tokens is not None:
            alien_tokens = alien_tokens_all[:self.top_k_tokens]
            ref_tokens = ref_tokens_all[:self.top_k_tokens]
            print(f"Using top {self.top_k_tokens} tokens (by frequency)")
        else:
            alien_tokens = alien_tokens_all
            ref_tokens = ref_tokens_all

        print(f"Alien vocabulary size: {len(alien_tokens)}")
        print(f"Reference vocabulary size: {len(ref_tokens)}")
        ...
        for i, alien_tok in enumerate(alien_tokens):
            ...
            for j, ref_tok in enumerate(ref_tokens):
                ...
                sim_matrix[i, j] = 0.5 * freq_sim + 0.5 * context_sim
    ...
    def solve_mapping(self) -> Dict[int, int]:
        ...
        for i, j in zip(row_ind, col_ind):
            recovered_mapping[alien_tokens[i]] = ref_tokens[j]

        print(f"Recovered {len(recovered_mapping)} token mappings")
        return recovered_mapping

    def apply_mapping(self, alien_sequences: List[List[int]], mapping: Dict[int, int]) -> List[List[int]]:
        """Apply the recovered mapping to alien token sequences."""
    ...
    def attack(self,
        ...
        Args:
            top_k_tokens: use only the top-k tokens by frequency (None uses all)
        """
        # Set top_k_tokens (override if passed as an argument)
...
class NGramAttackWithEvaluation:
    ...
    def __init__(self, n: int = 2, top_k_tokens: int = None):
        ...
        self.n = n
        self.top_k_tokens = top_k_tokens
        self.attack = NGramAttack(n=n, top_k_tokens=top_k_tokens)
    ...
    def run_full_experiment(self,
        ...
        """
        Run the complete N-gram attack experiment, including the true bijection.
...
def run_ngram_attack_experiment(
    ...
    Args:
        ...
        original_tokenizer: the original tokenizer
        alien_tokenizer: the alien tokenizer
        n: n-gram size
    ...
    """
    experiment = NGramAttackWithEvaluation(n=n, top_k_tokens=top_k_tokens)
