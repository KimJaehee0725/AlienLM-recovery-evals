...
###############################################################################
# Progressive LLM Attack Experiment (Non-Parallel Mode)
# Runs experiments sequentially over several few-shot sizes (0, 1, 3, 5, 10, 20).
# Non-parallel mode: zero-shot uses the parallel template, few-shot uses the
# non-parallel template.
# Reproduces Table 15 (non-parallel leakage, Appendix E.3.2).
###############################################################################
...
# Model settings
ATTACK_MODEL="gpt-5.1-2025-11-13"  # Empty string uses the config default
JUDGE_MODEL=""   # Empty string uses the config default

# Experiment settings
FEW_SHOT_SIZES="1 5 20"  # Few-shot sizes to test
N_TEST_SAMPLES=100          # Number of test samples
MAX_CONCURRENT_REQUESTS=32    # Limit on the number of concurrent requests
OUTPUT_DIR="./llm_attack_results/nonparallel_$(date +%Y%m%d_%H%M%S)"
...
# Non-parallel attack mode (default true)
NON_PARALLEL=true  # Use the non-parallel template for few-shot
...
usage() {
    ...
    echo "  -f, --few_shots LIST     Few-shot sizes (default: \"1 5 20\")"
    ...
    echo "  --parallel                Use parallel template (overrides default non-parallel mode)"
    echo "  --list_data               List available data files"
    echo "  -h, --help                Show this help message"
    echo ""
    echo "Note: Non-parallel mode uses non-parallel template for few-shot experiments."
    echo "      Zero-shot always uses parallel template regardless of mode."
    echo ""
...
if [ "$NON_PARALLEL" = true ]; then
    ...
    echo "      - Zero-shot: parallel template"
    echo "      - Few-shot: non-parallel template (leaked encrypted data context)"
else
    echo "Mode: PARALLEL ATTACK (all use parallel template)"
fi
echo "========================================================================"
echo ""
...
echo "Configuration:"
if [ "$NON_PARALLEL" = true ]; then
    echo "  Mode:                   Non-parallel attack (few-shot uses non-parallel template)"
else
    echo "  Mode:                   Parallel attack (all use parallel template)"
fi
